c Create LAMMPS data file (version 2001) for collection of
c   polymer bead-spring chains of various lengths and bead sizes
c Syntax: setup_chain < chain.def > data.file
c   chain.def is input file that specifies the chains (syntax in def files)
c   data.file in output file that will be input for LAMMPS
c include true flags in data file so chain can be unraveled later
      ...
      integer, allocatable :: truex(:),truey(:),truez(:)
 ...
 901  format(2f10.6,a)
 902  format(i3,f5.1)
 903  format(i6,i4,i4,f4.1,3f10.4,3i4)
...
c natoms = total # of monomers
      ...
      allocate(truex(natoms))
      allocate(truey(natoms))
      allocate(truez(natoms))
...
c 1st monomer is always in original box (true = 0)
...
c store new point
c if delta to previous bead is large, then increment/decrement true flag
...
c write out LAMMPS file
      ...
      write (6,900) 'LAMMPS Description'
      write (6,*)
      ...
      if (nbonds > 0) write (6,*) nbondtypes,' bond types'
      write (6,*)
      ...
      do i = 1,natoms
        write (6,903) i,molecule(i),type(i),0.0,x(i),y(i),z(i),
